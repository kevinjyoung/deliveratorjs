(function(){
  ...
  // DATA PARAMS
  // rid, address, deliveryTime, confirmUrl, isMobile
  ...
  // params: rid, template, extra, renderMenu, address, deliveryTime, confirmUrl
  ...
  //_render = function( res, rid, template, extra, renderMenu, address, deliveryTime, confirmUrl, cb) {
    ...
    this.getData( params.rid, function(err, data) {
      if(err){
        ...
        res.render( params.template, extra);
        if( _.isFunction(cb) ) { cb( null ); }
      }
    });
  };

  // params: rid, template, extra, address, dateTime, confirmUrl
  ...
  //_renderAddress = function( res, rid, template, extra, address, dateTime, confirmUrl ) {
    var isMobile = params.extra.isMobile ? params.extra.isMobile : false;
    var extraCss = params.extra.aff_data ? params.extra.aff_data.affSplashCss : null;
    var templateData = {
      rid           : params.rid,
      address       : JSON.stringify( params.address ),
      deliveryTime  : params.dateTime,
  ...
  // params: template, extra, address, menuUri, dateTime, renderList, filterRestaurants
  _renderRestaurants = function( res, params, cb ) {
  //_renderRestaurants = function( res, template, extra, address, menuUri, dateTime, renderList, filterRestaurants, cb ) {
    var self = this;
    this.getRestaurantList( params.address, params.dateTime, function(err, data){
      if(err){
        console.log(err);
        if( _.isFunction(cb) ) { cb( err ); }
      } else {
        ...
        if( params.renderList ){
          ...
          parameters.deliveryTime = params.dateTime;
        ...
        var isMobile = params.extra.isMobile ? params.extra.isMobile : false;
        var extraCss = params.extra.aff_data ? params.extra.aff_data.affRestCss : null;
        var templateData = {
  ...
  // params: template, extra, address, menuUri, dateTime
  ...
  // params: rid, template, extra, address, dateTime, tray, tip
