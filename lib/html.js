(function(){
  ...
  function Html(path, ordrinApi){
    ...
    this.getHead = function(rid, data, address, render, dateTime){
      if(isString(dateTime)){
        dateTime = '"'+dateTime+'"';
      }
      var hash = {path:path, data:JSON.stringify(data.menu), rid:rid, address:address, render:render, deliveryTime: dateTime};
      console.log(hash);
      return headTemplate(hash);
    }

    this.getSimpleHead = function(rid, address, dateTime){
      if(isString(dateTime)){
        dateTime = '"'+dateTime+'"';
      }
      return headTemplate({path:path, rid:rid, address:address, render:"menu", deliveryTime: dateTime});
    }
    ...
    var getRender = function(htmlObject){
      return function html_render(res, rid, template, extra, renderMenu, address, deliveryTime){
        htmlObject.getData(rid, function(err, data){
          if(err){
            ...
            extra.head = htmlObject.getHead(rid, data, JSON.stringify(address), renderMenu?"none":"menu", deliveryTime);
    ...
    var getRenderSimple = function(htmlObject){
      return function html_simple_render(res, rid, template, extra, address, dateTime){
        extra.head = htmlObject.getSimpleHead(rid, JSON.stringify(address), dateTime);
        res.render(template, extra);
      }
    }
    ...
    var getDefaultMenuMiddlewareGetter = function(htmlObject){
      return function(template, render_server, extra){
        ...
        return function html_default_menu_middleware(req, res){
          ...
          var dateTime = req.param("time", "ASAP");
          if(render_server){
            htmlObject.render(res, req.params.rid, template, extra, true, address, dateTime);
          } else {
            htmlObject.renderSimple(res, req.params.rid, template, extra, address, dateTime);
          }
        }
      }
    }
