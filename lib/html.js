(function(){
  ...
  function Html(path, ordrinApi){
    ...
    this.getRestaurantsHead = function(address, menuUri, dateTime, render, data, isMobile){
      if(isString(dateTime)){
        dateTime = '"'+dateTime+'"';
      }
      var hash = {path:path, data:JSON.stringify(data.restaurants), address:address, dateTime:dateTime, menu_uri:menuUri, render:render, isMobile : isMobile };
    ...
    var getRenderRestaurants = function(htmlObject){
      return function html_restaurants_render( res, template, extra, address, menuUri, dateTime, renderList, filterRestaurants ){
        htmlObject.getRestaurantList(address, dateTime, function(err, data){
          if(err){
            ...
            var isMobile = extra.isMobile ? extra.isMobile : false;
            extra.head = htmlObject.getRestaurantsHead(JSON.stringify(address), menuUri, dateTime, !renderList, data, isMobile);
            res.render(template, extra);
          }
        });
      }
