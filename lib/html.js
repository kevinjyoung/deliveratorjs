(function(){
  ...
  function Html(path, ordrinApi){
    ...
    this.getHead = function(rid, data, address, render, dateTime, confirmUrl, isMobile, affLogo ){
      if(isString(dateTime)){
        dateTime = '"'+dateTime+'"';
      }
      var hash = {path:path, details:JSON.stringify(data.details), data:JSON.stringify(data.menu), rid:rid, address:address, render:render, deliveryTime: dateTime, confirmUrl:confirmUrl, head_init:data.head_init, isMobile : isMobile, affLogo : affLogo };
      ...
      return function html_render(res, rid, template, extra, renderMenu, address, deliveryTime, confirmUrl){
        htmlObject.getData(rid, function(err, data){
          if(err){
            ...
            extra.head = htmlObject.getHead( rid, data, JSON.stringify(address), !renderMenu, deliveryTime, confirmUrl, isMobile, affLogo );
