(function(){
  ...
  function Html(path, ordrinApi){
    ...
    this.getSimpleHead = function(rid, address, dateTime, confirmUrl, isMobile){
      if(isString(dateTime)){
        dateTime = '"'+dateTime+'"';
      }
      return headTemplate({path:path, rid:rid, address:address, render:true, deliveryTime: dateTime, confirmUrl:confirmUrl, isMobile : isMobile });
    }
    ...
    var getRenderSimple = function(htmlObject){
      return function html_simple_render(res, rid, template, extra, address, dateTime, confirmUrl){
        var isMobile = extra.isMobile ? extra.isMobile : false;
        extra.head = htmlObject.getSimpleHead(rid, JSON.stringify(address), dateTime, confirmUrl, isMobile);
        res.render(template, extra);
      }
    }
