(function(){
  ...
  function Html(path, ordrinApi){
    ...
    this.getHead = function(rid, data, address, render, dateTime, confirmUrl, isMobile){
      if(isString(dateTime)){
        dateTime = '"'+dateTime+'"';
      }
      var hash = {path:path, details:JSON.stringify(data.details), data:JSON.stringify(data.menu), rid:rid, address:address, render:render, deliveryTime: dateTime, confirmUrl:confirmUrl, head_init:data.head_init, isMobile : isMobile };
    ...
    this.getSimpleHead = function( rid, address, dateTime, confirmUrl, isMobile, noAngular ){
      ...
      return headTemplate({path:path, rid:rid, address:address, render:true, deliveryTime: dateTime, confirmUrl:confirmUrl, isMobile : isMobile, noAngular: noAngular });
    }

    this.getAddressHead = function( rid, address, dateTime, confirmUrl, isMobile, noAngular ){
      if(isString(dateTime)){
        dateTime = '"'+dateTime+'"';
      }
      return addressHeadTemplate({path:path, rid:rid, address:address, render:true, deliveryTime: dateTime, confirmUrl:confirmUrl, isMobile : isMobile, noAngular: noAngular });
    }
    ...
    this.getRestaurantsHead = function( address, menuUri, dateTime, render, data, isMobile, additQs ){
      if(isString(dateTime)){
        dateTime = '"'+dateTime+'"';
      }
      var hash = {path:path, data:JSON.stringify(data.restaurants), address:address, dateTime:dateTime, menu_uri:menuUri, render:render, isMobile : isMobile, additQs : additQs };
      ...
      return function html_render(res, rid, template, extra, renderMenu, address, deliveryTime, confirmUrl){
        htmlObject.getData(rid, function(err, data){
          if(err){
            ...
            extra.head = htmlObject.getHead(rid, data, JSON.stringify(address), !renderMenu, deliveryTime, confirmUrl, isMobile);
    ...
    var getRenderRestaurants = function(htmlObject){
      return function html_restaurants_render( res, template, extra, address, menuUri, dateTime, renderList, filterRestaurants, additQs ){
        htmlObject.getRestaurantList(address, dateTime, function(err, data){
          if(err){
            ...
            var isMobile = extra.isMobile ? extra.isMobile : false;
            extra.head = htmlObject.getRestaurantsHead( JSON.stringify(address), menuUri, dateTime, !renderList, data, isMobile, additQs );
    ...
    var getRenderAddress = function(htmlObject){
      return function html_simple_render(res, rid, template, extra, address, dateTime, confirmUrl){
        var isMobile = extra.isMobile ? extra.isMobile : false;
        var noAngular = extra.noAngular ? extra.noAngular : false;
        extra.head = htmlObject.getAddressHead(rid, JSON.stringify(address), dateTime, confirmUrl, isMobile, noAngular);
        res.render(template, extra);
      }
    }
